...
Let's connect via: `ftp 10.129.143.56` and sign in as an `anonymous` user. Bingo, now just find the flag.
...
We can list all the smb sharenames by `smbclient -L 10.129.24.249`. Now let's try to connect to all of them via `smbclient \\\\10.129.24.249\\<Sharename>`
